    ...
    <b-container class="bv-example-row">
      <b-row class="text-left">
        ...
        <center>
          ...
          <h5>แก้ไขการสั่งซื้อ รายการที่ : {{ orderlist.id }} </h5><hr>
        </center>
        
          <form class="box1" v-on:submit.prevent="editOrderlist">
            ...
            <p>
              ...
              <b-button variant="secondary" v-on:click="navigateTo('/orderlists')"><i class="fa fa-arrow-circle-left">ย้อนกลับ</i></b-button>
            </p>
          </form>
          </b-col>
        <b-col> </b-col>
...
export default {
  ...
  methods: {
    async editOrderlist() {
      try {
        await OrderlistsService.put(this.orderlist);
        this.$router.push({
          name: "orderlists",
        });
      } catch (err) {
  ...
  async created() {
    try {
      let orderlistId = this.$route.params.orderlistId;
      this.orderlist = (await OrderlistsService.show(orderlistId)).data;
